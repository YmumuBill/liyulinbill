...
class BaseController extends Controller {
    ...
    private function checkAuth(){
        ...
        /*admin  id=1 超级管理员有所有的权限 先判断node节点 再判断group节点*/
        if($adm_name != C("DEFAULT_ADMIN")&&$adm_id != 1&&CONTROLLER_NAME!='Index'){
            ...
            if($count>0){
                ...
                if($count2==0){
                    ...
                    if($module_count == 0) {
                        if($ajax == 1){
                            echo json_encode(array("status"=>0,"info"=>"没有权限访问"));die;
                        }
                        else{
        ...
        if(CONTROLLER_NAME=="Log"&&$adm_id!=1){
            if($ajax == 1){
                echo json_encode(array("status"=>0,"info"=>"没有权限访问"));die;
            }
            else{
