...
<div id="body">
    ...
    <div class="ibox animated fadeInRight">
        <div class="ibox-title">
            <div>
                <a class="btn btn-success" href="<?php echo U('Figures/add');?>">+新增</a>
            </div>
        </div>
        <div class="ibox-content">
            <table class="table table-striped table-bordered table-hover dataTables-example">
                ...
                <?php if(is_array($lists)): foreach($lists as $key=>$item): ?><tr>
                        <td>
                            <input type="checkbox" class="key" value="<?php echo ($item["id"]); ?>">
                        </td>
                        <td><?php echo ($item["id"]); ?></td>
                        <td><?php echo ($item["name"]); ?></td>
                        <td><?php echo ($item["sex"]); ?></td>
                        <td><?php echo ($item["occupation"]); ?></td>
                        <td><?php echo (date("Y年m月d日",$item["birthday"])); ?></td>
                        <td data-id="<?php echo ($item["id"]); ?>">
                            <a href="<?php echo U('Figures/edit',array('id'=>$item['id']));?>" class="btn btn-white">
                                <i class="icon-edit"></i>编辑
                            </a>
                            <?php if($item['is_recommend'] == 0): ?><button class="btn btn-success" action="recommend" data-type="1">
                                    <i class="icon-thumbs-up"></i>推荐
                                </button>
                                <?php else: ?>
                                <button class="btn btn-success" action="recommend" data-type="0">
                                    <i class="icon-undo"></i>取消推荐
                                </button><?php endif; ?>
                            <button class="btn btn-danger" action="del"><i class="icon-trash"></i>删除</button>
                        </td>
